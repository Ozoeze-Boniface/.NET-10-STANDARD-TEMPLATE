        ...
        if (authorizeAttributes.Any())
        {
            ...
            if (authorizeAttributesWithRoles.Any())
            {
                ...
                foreach (var roles in authorizeAttributesWithRoles.Select(a => a.Roles.Split(',')))
                {
                    ...
                    {
                        var isInRole = await this._identityService.IsInRoleAsync(this._user.Id, role.Trim());
                        if (isInRole)
                        {
                            authorized = true;
                            break;
            ...
            if (authorizeAttributesWithPolicies.Any())
            {
                ...
                {
                    var authorized = await this._identityService.AuthorizeAsync(this._user.Id, policy);
